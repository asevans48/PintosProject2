...
tid_t
 process_execute (const char *file_name){
  ...
  char * svptr;
  char * file_name_parse = strtok_r((char *) file_name," ",&svptr);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (file_name_parse, PRI_DEFAULT, start_process, fn_copy);
...
void _exit_process(int status){
  ...
  struct file_elem * fel = get_file(t->fd);
  if(t->tid == 1){
    return;
  }

  struct list * files = &thread_current()->file_list;
  struct list_elem * e = list_back(files);
...
static bool setup_stack (void **esp,const char * args,char ** svptr);
 ...
 */
bool
load (const char * args, void (**eip) (void), void **esp, char ** svptr)
{
  struct thread *t = thread_current ();
...
static bool
setup_stack (void **esp,const char * args,char ** svptr)
{
  uint8_t *kpage;
  bool success = false;
  ...
  if (kpage != NULL)
  {
    ...
    if (success) {
      *esp = PHYS_BASE;
      ...
      for(token = (char *) args; token != NULL; token = strtok_r(NULL," ",svptr)){
        *esp -= strlen(token) + 1;
        argsv[argsc] = *esp;
        argsc++;
        ...
        if(argsc >= currSize){
          currSize *= 2;
          argsv = realloc(argsv, sizeof(char *) * currSize);
        }
      ...
      if(offset) {
        *esp -= offset;
        memcpy(*esp, &argsv[argsc],offset);
      }

      for(int i = argsc; i >= 0; i--){
        *esp -= sizeof(char *);
        memcpy(*esp,&argsv[i],sizeof(char *));
      }
